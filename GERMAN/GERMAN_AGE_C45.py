...
def devide_data(data):
    a = data.iloc[:,-1].value_counts().to_dict() 
    for k, v in a.items(): 
        a[k] = v/10        
...
def search_best_attribute(data, available_attributes, categorical_features, weights):
    ...
    for i in range(len(predictors)):        
        if predictors[i] == True:
            ...
            if c[1][1] > 0: #include only positive gains
                average_gain += c[1][1]
                candidates.append(c)
    if len(candidates) == 0:
...
def build_decision_nodes(metadata, data, df, pro_index, categorical_features, available_attributes, pri_val, unpri_val, pri_class, tree=None, parent_majority=None, weights=None):
    ...
    index, info = search_best_attribute(data, available_attributes, categorical_features, weights)

    if index != None:
        attribute = df.columns[index]
    else:
        attribute = None
    ...
    if is_numeric_dtype(df.iloc[:,index]):
        #lower partition
        count = weights[data[:,index] <= info[2]].sum()
