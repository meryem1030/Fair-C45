...
def load_csv(path, pri_class, non_pri_class,  label_wanted, CLASS, drop_attribute = None,operation = None):

    data = pd.read_csv(path)

    if drop_attribute != None:
        data =  data.drop(columns = [drop_attribute])

    #0 is positive class, 1 is negative class
    for col in data.columns:
        data[col] = data[col].replace(r'?', np.nan)
    for col in data.columns:
        data[col] = data[col].replace(r'N', np.nan)
        
    if operation == '==':
        data[CLASS]=np.where(data[CLASS] ==  label_wanted, pri_class, non_pri_class)
    else:
        data[CLASS]=np.where(data[CLASS] >=  label_wanted, pri_class, non_pri_class)
    #data['CLASS'] = data.loc[:,CLASS]
...
df,categorical_features, metadata_original, data = load_csv(r'german_credit.csv', 2, 1, 2,'CreditStatus',operation = '==')
...
def math_model(np_table):
    ...
    if len(np_table) == 0:
        ...
        print(new_table)
        a = len(np_table[:,0])
        f = len(np_table[0,1:])-1
        candidates = []
        for i in range(len(new_table)):
            fair_total = new_table[i,1] + new_table[i,2] + new_table[i,3] + new_table[i,5]
            if max(new_table[i,1:]) - min(new_table[i,1:]) != 0:
                epsilon = (max(new_table[i,1:]) - min(new_table[i,1:])) / (max(new_table[i,1:]) + min(new_table[i,1:]))
            else:
                epsilon = (max(new_table[i,1:]) - 0) / (max(new_table[i,1:]) + 0)  
            if fair_total <= (epsilon * a * f) + 1*4:
                candidates.append(i) ##add index of attribute index 
        selection_idx = []
        selection = []
        for j in candidates:
            obj_try = new_table[j, 4]*(new_table[j,1] + new_table[j,2] + new_table[j,3] + new_table[j,5])
            selection_idx.append(j)
            selection.append(obj_try)
        store = []
        find_min = min(selection)
        for k in range(len(selection)):
            if selection[k] == find_min:
                store.append(selection_idx[k]) # store the attrbute index
        if len(store) == 1:
            selected_attribute = new_table[store[0],0]
        else:
            selected_attribute = new_table[random.choice(store),0]
...
def DI_CV_Calculate(data, test, pro_index, categorical_features, pri_val, unpri_val, pri_class, user_defined_threshold):
...
def consistency_pre(data_split, categorical_features, y_split, pro_index, k=5):
    ...
    if len(data_split) > 0:
        ...
        y_split = 0
        return 0
def consistency_test(data, test, pro_index, categorical_features, pri_val, unpri_val, pri_class,user_defined_threshold):

    weights = np.ones(len(test))
    try:
        pri_data = DI_CV_Calculate(data, test, pro_index, categorical_features, pri_val, unpri_val, pri_class,user_defined_threshold)[0]
        X_priviliged = pri_data[~np.isnan(pri_data).any(axis=1)][:,:-1]
        y_priviliged = pri_data[~np.isnan(pri_data).any(axis=1)][:,-1]
    except:
        pri_data = []
    ...
    try:
        unpri_data = DI_CV_Calculate(data, test, pro_index, categorical_features, pri_val, unpri_val, pri_class,user_defined_threshold)[1]
        X_unpriviliged = unpri_data[~np.isnan(unpri_data).any(axis=1)][:,:-1]
        y_unpriviliged = unpri_data[~np.isnan(unpri_data).any(axis=1)][:,-1]
    except:
        unpri_data = []
...
def DT_LEFT_TEST(data, test, pro_index, index_att, available_attributes, categorical_features, pri_val, unpri_val, pri_class, user_defined_threshold):
    weights = np.ones(len(data))
    PRI = DI_CV_Calculate(data, test, pro_index, categorical_features, pri_val, unpri_val, pri_class, user_defined_threshold)[0]
    UNPRI = DI_CV_Calculate(data, test, pro_index, categorical_features, pri_val, unpri_val, pri_class, user_defined_threshold)[1]
    weights_pri = np.ones(len(PRI))
    weights_unpri = np.ones(len(UNPRI))
    test_original = test.copy()
    data_original = data
...
def Treatment_test_diffs(data, test, categorical_features, available_attributes, pro_index, pri_val, unpri_val, pri_class,user_defined_threshold):
    ...
    for i in range(len(test[0,:-2])):
        if np.unique(test[:,i][~pd.isna(test[:,i])]).tolist() != []:
            if i != pro_index:
                ind.append(i)
                DT = abs(DT_LEFT_TEST(data, test, pro_index, i, available_attributes, categorical_features, pri_val, unpri_val, pri_class, user_defined_threshold) - DT_RIGHT_RIGHT(data, test, i, available_attributes, categorical_features, pri_class))
                DTs.append(DT)
...
def run(metadata, data, df, part, pro_index, categorical_features, pri_val, unpri_val, pri_class, non_pri_class,user_defined_threshold):
    accuracy = []
    Disparate_impacts = []
    DTs = []
    CV_scores = []
    ...
    for i in range(len(part)): 
        ...
        DI = DI_CV_Calculate(data, test, pro_index, categorical_features, pri_val, unpri_val, pri_class,user_defined_threshold)[2] 
        Disparate_impacts.append(DI)
        CV = DI_CV_Calculate(data, test, pro_index, categorical_features, pri_val, unpri_val, pri_class,user_defined_threshold)[3] 
        CV_scores.append(CV)
        DT = Treatment_test_diffs(data, test, categorical_features, available_attributes, pro_index, pri_val, unpri_val, pri_class,user_defined_threshold)[1]
        DTs.append(DT)
        cons= consistency_test(data, test, pro_index, categorical_features, pri_val, unpri_val, pri_class,user_defined_threshold)
...
run(metadata, data, df, part, 13,categorical_features, 1, 2, 2, 1,user_defined_threshold=24)
