...
df,categorical_features, metadata_original, data = load_csv(r'student_mat_binary.csv')
...
def DI_CV_Calculate(data, test, pro_index, categorical_features, pri_val, unpri_val, pri_class, user_defined_threshold):
    ...
    if pro_index in categorical_features: #transform in binary case if the index in categorical list
        ...
        test_original[pd.notna(test_original[:,pro_index]), pro_index] = np.where(test_original[pd.notna(test_original[:,pro_index]), pro_index] > att_threshold, unpri_val, pri_val)
        #print(att_threshold)
...
run(metadata, data, df, part, 2,categorical_features, 1, 2, 1, 0,user_defined_threshold=16)
