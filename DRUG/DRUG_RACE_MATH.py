...
def load_csv(path, pri_class, non_pri_class,  label_wanted, CLASS, drop_attribute = None,operation = None):
    ...
    #1:never used, 0:ever used
    data['CLASS'] = data.loc[:,'Meth ']
    data =  data.drop(columns = ['Meth '])
    if operation == '==':
...
df,categorical_features, metadata_original, data = load_csv(r'drug_consumption.csv', 1, 0, 'CL0','CLASS', 'ID',operation = '==')
...
run(metadata, data, df, part, 4,categorical_features, 2, 1, 1, 0)
