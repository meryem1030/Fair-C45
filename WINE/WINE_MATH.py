...
def load_csv(path):
    ...
    data['binned_quality']=np.where(data['binned_quality'] == 'good', 1, 0)
...
df,categorical_features, metadata_original, data = load_csv(r'wine.csv')
...
run(metadata, data, df, part, 11,categorical_features, 2, 1, 1, 0)
