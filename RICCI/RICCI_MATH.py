...
def load_csv(path, pri_class, non_pri_class,  label_wanted, CLASS, drop_attribute = None,operation = None):
    ...
    #data.iloc[:,] = np.where(data.iloc[:,1] == -0.48246 , 1, 0) #1:WHITE, 0:NON-WHITE
...
df,categorical_features, metadata_original, data = load_csv(r'ricci.csv', 1, 0, 70,'Combine', 'ID',operation = '>=')
...
run(metadata, data, df, part, 0,categorical_features, 1, 3, 1, 0)
