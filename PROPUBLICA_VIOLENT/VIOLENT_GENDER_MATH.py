...
def load_csv(path):
    ...
    drop_columns = ['id', 'name', 'first', 'last','compas_screening_date', 'dob', 'days_b_screening_arrest', 'c_jail_in', 'c_jail_out',
                    'c_case_number', 'c_offense_date','c_arrest_date', 'c_days_from_compas', 'is_recid', 'r_case_number','r_charge_degree', 
                    'r_days_from_arrest', 'r_offense_date', 'r_charge_desc','r_jail_in', 'r_jail_out', 'violent_recid', 'is_violent_recid',
                    'vr_case_number', 'vr_charge_degree', 'vr_offense_date', 'vr_charge_desc','type_of_assessment', 'decile_score.1', 'screening_date', 'v_type_of_assessment',
                    'v_decile_score', 'v_screening_date', 'in_custody','out_custody', 'priors_count.1', 'start', 'end','event', 'two_year_recid']
...
df,categorical_features, metadata_original, data = load_csv(r'propublica_violent_recidivism.csv')
...
def consistency(data_split, categorical_features, y_split, pro_index, k=5):
    ...
    cols = []
    if len(data_split) > 0:
        [cols.append(i) for i in categorical_features if i < data_split.shape[1]]
...
def consistency_data(data, pro_index, categorical_features, pri_val, unpri_val, pri_class):
    ...
    try:
        ...
        X_priviliged = pri_data[:, ~np.isnan(pri_data).any(axis=0)][:,:-1]
        
        y_priviliged = pri_data[:, ~np.isnan(pri_data).any(axis=0)][:,-1]
    except:
        pri_data = []
        X_priviliged = []
        y_priviliged = []
    ...
    try:
        ...
        X_unpriviliged = unpri_data[:,~np.isnan(unpri_data).any(axis=0)][:,:-1]
        y_unpriviliged = unpri_data[:,~np.isnan(unpri_data).any(axis=0)][:,-1]
    except:
        unpri_data = []
        X_unpriviliged = []
        y_unpriviliged = []
...
run(metadata, data, df, part, 0,categorical_features, 1, 2, 0, 1)
