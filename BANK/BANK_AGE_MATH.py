...
df,categorical_features, metadata_original, data = load_csv(r'bank-full.csv')
df
...
def DI_CV_Calculate(data, test, pro_index, categorical_features, pri_val, unpri_val, pri_class, user_defined_threshold=None):
    ...
    if pro_index in categorical_features: #transform in binary case if the index in categorical list
        ...
        test_original[(test_original[:,pro_index] >=33) & (test_original[:,pro_index] <60), pro_index] = 1
        test_original[(test_original[:,pro_index] != 1), pro_index] = 0
...
run(metadata, data, df, part, 0,categorical_features, 1, 0, 1, 0)
