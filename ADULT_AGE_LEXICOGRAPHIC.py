...
def consistency_data(data, pro_index, categorical_features, pri_val, unpri_val, pri_class):
    ...
    try:
        ...
        X_priviliged = []
    print('att selection len priviliged data_split:', len(X_priviliged))
    try: 
        y_priviliged = pri_data[~np.isnan(pri_data).any(axis=1)][:,-1]
    except:
        y_priviliged = []
    print('att selection len y pri:', len(y_priviliged))
    ...
    try:
        ...
        X_unpriviliged = []
    print('att selection len unpriviliged data_split:', len(X_unpriviliged))
    try:
        y_unpriviliged = unpri_data[~np.isnan(unpri_data).any(axis=1)][:,-1]
    except:
        y_unpriviliged = []
    print('att selection len y unpri:', len(y_unpriviliged))
...
def search_lexi(np_table):    
    if len(np_table) == 0:
        ...
        if lexicographical(new_table) == None:
            ...
            print('random_choose:', random_choose)
