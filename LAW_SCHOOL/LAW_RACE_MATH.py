...
def load_csv(path):
    ...
    #data.iloc[:,4] = np.where(data.iloc[:,4] == -0.31685, 1, 0) #1:WHITE, 0:NON-WHITE
    data['CLASS'] = data.loc[:,'pass_bar']
    data =  data.drop(columns = ['pass_bar'])
    
    #data.iloc[:,-1] = np.where(data.iloc[:,-1] == 'CL0', 1, 0) #1:never used, 0:ever used
    df = data
...
df,categorical_features, metadata_original, data = load_csv(r'bar_pass.csv')


# In[5]:


def convert_categories(data, categorical_features, convert_cols):
    count = 0
...
convert_cols = [2, 3, 9, 13, 14]
data, categorical_features = convert_categories(data, categorical_features, convert_cols)
...
def numpy_table(data, pro_index, categorical_features, available_attributes, pri_val, unpri_val, pri_class, weights):
    ...
    return np_table
#weights = np.ones(len(data))
#t = numpy_table(data, 9, categorical_features, available_attributes, 1, 2, 2, weights)
...
def math_model(np_table):
    print('np_table:',np_table)
    if len(np_table) == 0:
        return None
    else:
        newsss = distance(np_table)
        print('newss:',newsss)
        if len(drop_dominateds(np_table)) >= 2:
            print('drop wanted:', len(np_table), len(drop_dominateds(np_table)))
            new_table = drop_dominateds(np_table)
            print('wanted new_table', len(new_table))
        else:
...
run(metadata, data, df, part, 2,categorical_features, 7, 0, 1, 0)
